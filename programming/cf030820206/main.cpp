//1915001 Debojyoti Das NIT Silchar 3hrs
#include<bits/stdc++.h>
using namespace std;
...
l n,ans=0;
cin>>n;
string s,t,R="abacaba";
cin>>s;
for(int i=0;i+6<n;i++) {
			bool valid=true;
			for(int j=0;j<7;j++) if(s[i+j]!='?' && s[i+j]!=R[j]) valid=false;
			if(valid) {
				int cnt=0;
				t=s;
				for(int j=0;j<7;j++) t[i+j]=R[j];
				for(auto &c: t) if(c=='?') c='z';
				for(int j=0;j+6<n;j++) {
					valid=true;
					for(int k=0;k<7;k++) if(t[j+k]!=R[k]) valid=false;
					cnt+=valid;
				}
				if(cnt==1) {
					ans=true;
