...
fo(i,n)cin>>a[i];
fo(i,n)cin>>b[i];
if(find(a.begin(),a.end(),1LL)!=a.end()){
    cout<<n;
    return;
}
l f=0;
l temp=find(b.begin(),b.end(),1LL)-b.begin();
cout<<temp;nl;
for(l i=temp;i<n-1;i++){
    if(b[i]+1!=b[i+1]){
        f=1;
        break;
    }
}
if(f==0)cout<<n-(find(b.begin(),b.end(),1LL)-b.begin())+1;
if(f==1) {
    cout<<find(b.begin(),b.end(),1LL)-b.begin()+1+n;
}
}
