...
void solve(){
    ...
    vl vL,vR,v(n);
    fo(i,n)cin>>v[i];
    stack<pair<l,l>>NSL,NSR;
    for(l i=0;i<n;i++){
        if(NSL.size()==0){
            vL.push_back(-1);
        }else if(v[i]<=NSL.top().first&&NSL.size()>0){
        while(NSL.top().first>=v[i]&&NSL.size()>0){
            NLL.pop();
        }
        if(NSL.size()==0){
            vL.push_back(-1);
        }else{
        vL.push_back(NSL.top().second);
        }
        }else{
            vL.push_back(-1);
        }
        ...
        if(NSR.size()==0){
            vR.push_back(n);
        }else if(v[i]<=NSR.top().first&&NSR.size()>0){
        while(NSR.top().first>=v[i]&&NSR.size()>0){
            NSR.pop();
        }
        if(NSR.size()==0){
            vR.push_back(n);
        }else{
        vR.push_back(NSR.top().second);
        }
        }else{
            vR.push_back(n);
        }
