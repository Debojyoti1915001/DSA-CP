...
public:
    ...
    int idx=0;
    Node* solve(int pre[], int size,int mn,int mx){
        if(idx>size)return NULL;
        if(pre[idx]<mn||pre[idx]>mx)return NULL;
        Node* root=newNode(pre[idx++]);
        root->left=solve(pre,size,mn,root->data);
        root->right=solve(pre,size,root->data,mx);
        return root;
    }
    Node* post_order(int pre[], int size)
    {
        //code here
        return solve(pre,size-1,INT_MIN,INT_MAX);
    }
};
